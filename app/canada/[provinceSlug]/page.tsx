...
export async function generateMetadata({
  ...
  const { provinceName, totalFacilities } = await getProvinceSummary(
    safeSlug,
  );

  const title = `Funeral Homes in ${provinceName}, Canada | Funeral Home Directories`;
  const descriptor = `Find ${totalFacilities.toLocaleString()} funeral homes in ${provinceName}, Canada. Compare services and contact details. Verified listings with ratings and reviews.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${provinceName} funeral home directory preview`,
        },
      ],
    },
  };
...
export default async function ProvincePage({ params }: ProvincePageProps) {
  ...
  const funeralFocusText =
    "funeral homes, mortuaries, crematories, memorial chapels, and cemeteries";
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `How many funeral homes are in ${provinceName}?`,
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: `What types of funeral providers are listed in ${provinceName}?`,
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "How are funeral homes selected for this directory?",
        acceptedAnswer: {
          "@type": "Answer",
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Funeral Homes in ${provinceName}, Canada`,
    ...
    about: [
      { "@type": "Thing", name: `${provinceName} funeral homes` },
      ...funeralCategorySchemaThings(),
    ],
    speakable: {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      >
        Get your funeral home featured — view pricing &amp; benefits →
      </Link>
      <section className="rounded-2xl bg-surface-muted px-5 py-6 text-foreground shadow-lg shadow-navy/10 ring-1 ring-gold/40 sm:px-8 sm:py-8">
        ...
        <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">
          Funeral Homes in {provinceName}, Canada
        </h1>
        <p className="mt-3 max-w-2xl text-sm text-foreground/80">
          Explore {funeralFocusText} across {provinceName}, including major
          city areas such as {majorCitiesText}. Use this page to find funeral homes by
          city.
        </p>
        ...
        return (
          <section className="mt-8 rounded-2xl border-2 border-teal/20 bg-teal/5 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-600">
              Featured funeral homes in {provinceName} — verified listings with
              priority placement.
            </p>
      ...
      <section className="mt-8 rounded-2xl border-2 border-navy/20 bg-background p-6 space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-navy border-b-2 border-teal/50 pb-1 inline-block">
              Funeral homes by city in {provinceName}
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-slate-600">
              Choose a city to browse funeral homes, mortuaries, and crematories in{" "}
              {provinceName}, including cremation, burial, and memorial service providers.
            </p>
          </div>
        ...
        {cities.length === 0 ? (
          <p className="text-sm text-slate-600">
            We don&apos;t have funeral homes listed for {provinceName} yet. As new data
